...
def process_commercial_resistors(
    csv_files, commercial_resistors=[10, 12, 15, 18, 22, 27, 33, 39, 47, 56]
):
    """
    ...
    for file in csv_files:
        ...
        commercial_resistor_df = df.applymap(
            lambda x: (
                map_to_commercial_resistor(x, commercial_resistors)
                if x != "NULL"
                else "NULL"
            )
        )
        commercial_resistor_df.to_csv(
            f"commercially_available_resistor_values_{file}", header=None, index=False
        )
